...
class SwPipe():
    ...
    def __run(self,commande,shell,stdin,stdout,stderr,readyfunc):
        ...
        self._flag = False
        self._exitfunc(LoopException("While Loop break\r\n"))   #正常退出
    ...
    def end(self):
        """ 结束线程 """
        command = 'taskkill /F /IM ccyclone.exe'
        os.system(command)
...
if __name__ == '__main__':#那么我们来开始使用它吧
    ...
    def ready():#线程就绪反馈函数
        e.write("position fen rnbakabnr/9/1c5c1/p1p1p1p1p/9/9/P1P1P1P1P/1C5C1/9/RNBAKABNR w - - 0 1")  #执行
        e.write("go depth 17")  #执行
       

    e = SwPipe("D:/ccyclone/ccyclone.exe",event,exit,ready)
    e.start()
